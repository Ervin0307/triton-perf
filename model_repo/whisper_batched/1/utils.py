...
def load_model(model_name_or_path, model_dtype, model_kwargs, logger, use_hpu_graphs=True):
    logger.info(f"Loading model : {model_name_or_path}")

    model = AutoModelForSpeechSeq2Seq.from_pretrained(
        model_name_or_path, torch_dtype=model_dtype,
        low_cpu_mem_usage=True, use_safetensors=True, attn_implementation="sdpa",
        **model_kwargs
    )
...
def initialize_model_n_processor(args, logger):
    ...
    get_repo_root(args.model_name_or_path,
                  local_rank=args.local_rank, token=args.token)
    ...
    model = load_model(model_name_or_path, model_dtype,
                       model_kwargs, logger, args.use_hpu_graphs)
